...
exports.ordersFilter = function(req, res, next) {
	...
	if(req.query && req.query.keytype == "sfer"){
		...
		hrOrderFindSfers(req, res, next, condition);
	} else if(req.query && req.query.keytype == "vder"){
		condition.keytype = "order";
		condition.keyword = "";
		condition.symPul = "$eq";
		condition.condPul = req.query.keyword;
		hrOrderFindVders(req, res, next, condition);
	} else {
		condition.symPul = '$ne';
		condition.condPul = randID;
		hrOrderFindOrders(req, res, next, condition);
	}
}
hrOrderFindSfers = function(req, res, next, condition) {
...
hrOrderFindVders = function(req, res, next, condition) {
...
hrOrderFindOrders = function(req, res, next, condition) {
	Order.count({
...
exports.orders = function(req, res) {
	...
	res.render('./sfer/hrer/order/list', list)
}
...
exports.orderAdd =function(req, res) {
	res.render('./sfer/hrer/order/add', {
		title: 'Add Order',
	...
	if(isNaN(objBody.price)){
		...
		_object.save(function(err, objSave) {
			if(err) {
				...
				res.redirect('/hrOrder/'+objSave._id)
			}
		})
	}
}

hrAddPayFunc = function(objBody, object) {
...
exports.order = function(req, res) {
	...
	res.render('./sfer/hrer/order/detail', {
		title: 'hrOrder Infomation',
		crSfer : req.session.crSfer,
		object: objBody,
		today: today,
		weekday: weekday
...
exports.orderUp = function(req, res) {
	...
	if(objBody.status == 0) {
		...
		res.render('./sfer/hrer/order/update', {
			title: 'hrOrder Update',
			action: '/hrOrderUpd',
			crSfer : req.session.crSfer,
			object: objBody,
			today: today,
	...
	Order.findOne({_id: objBody._id}, function(err, object) {
		...
		if(!object) {
			...
			_object.save(function(err, objSave) {
				if(err) {
					console.log(err);
					info = "hrOrderUpd order save error, Please Contact Manager";
					Index.hrOptionWrong(req, res, info);
				} else {
					res.redirect('/hrOrder/'+objSave._id)
				}
	...
	Order.findOne({_id: objBody._id}, function(err, object) {
		...
		if(!object) {
			...
			if(isNaN(parseFloat(objBody.price)) || isNaN(parseFloat(objBody.acPrice)) || isNaN(parseFloat(objBody.saPrice))){
				...
				let _object = _.extend(object, objBody)
				hrUpdPayFunc(req, res, objBody, _object)
...
hrUpdPayFunc = function(req, res, objBody, object) {
	Pay.findOne({_id: object.payAc}, function(err, payAc) {
		if(err) {
			console.log(err);
			info = "hrOrderUpd payAc find id error, Please Contact Manager";
			Index.hrOptionWrong(req, res, info);
		} else if(!payAc) {
			info = "hrOrderUpd payAc find error, Please Contact Manager";
			Index.hrOptionWrong(req, res, info);
		} else {
			...
			payAc.save(function(err, payAcSave) {
				...
				Pay.findOne({_id: object.paySa}, function(err, paySa) {
					if(err) {
						console.log(err);
						info = "hrOrderUpd paySa find id error, Please Contact Manager";
						Index.hrOptionWrong(req, res, info);
					} else if(!paySa) {
						info = "hrOrderUpd paySa find error, Please Contact Manager";
						Index.hrOptionWrong(req, res, info);
					} else {
...
exports.orderDel = function(req, res) {
	...
	if(objBody.status == 0) {
		...
		Order.remove({_id: objBody._id}, function(err, hrOrderRm) {
			if(err) console.log(err);
			res.redirect('/hrOrders')
		})
	} else {
