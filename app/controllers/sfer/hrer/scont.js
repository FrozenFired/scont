...
exports.sconts = function(req, res) {
	res.render('./sfer/hrer/scont/list', req.body.list)
}
...
exports.scontAdd = function(req, res) {
	Nation.find(function(err, nations) {
		if(err) console.log(err);
		res.render('./sfer/hrer/scont/add', {
			title: 'Discount Add',
...
exports.addScont = function(req, res) {
	hrCreateBrand(req, res);
}
hrCreateBrand = function(req, res) {
...
hrCreateScont = function(req, res) {
		...
		.exec(function(err, sconts) {
			...
			if(sconts.length > 0) {
				...
				_object.save(function(err, objSave) {
					...
					res.redirect('/hrScont/' + objSave._id)
				})
			}
		})
	}
...
exports.scont = function(req, res){
	...
	res.render('./sfer/hrer/scont/detail', {
...
let rtHrPath = require('path').join(__dirname, "../../../../");
...
exports.scontPdf = function(req, res) {
	...
	// let pug = require('pug');
	if(brand && vendor) {
		let hc = require('pug').renderFile(rtHrPath + 'views/zzPdf/scont/detail.pug', {
			publicPath: "file://"+rtHrPath + 'public',
