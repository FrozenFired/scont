...
exports.orderAdd =function(req, res) {
	res.render('./sfer/lger/order/add', {
		title: 'Add Order',
...
lgAddPayFunc = function(objBody, object) {
	...
	Order.findOne({_id: objBody._id}, function(err, object) {
		...
		if(!object) {
			...
			if(isNaN(parseFloat(objBody.price)) || isNaN(parseFloat(objBody.acPrice)) || isNaN(parseFloat(objBody.saPrice))){
				...
				let _object = _.extend(object, objBody)
				lgUpdPayFunc(req, res, objBody, _object)
...
lgUpdPayFunc = function(req, res, objBody, object) {
	Pay.findOne({_id: object.payAc}, function(err, payAc) {
		if(err) {
			console.log(err);
			info = "lgOrderUpd payAc find id error, Please Contact Manager";
			Index.lgOptionWrong(req, res, info);
		} else if(!payAc) {
			info = "lgOrderUpd payAc find error, Please Contact Manager";
			Index.lgOptionWrong(req, res, info);
		} else {
			...
			payAc.save(function(err, payAcSave) {
				...
				Pay.findOne({_id: object.paySa}, function(err, paySa) {
					if(err) {
						console.log(err);
						info = "lgOrderUpd paySa find id error, Please Contact Manager";
						Index.lgOptionWrong(req, res, info);
					} else if(!paySa) {
						info = "lgOrderUpd paySa find error, Please Contact Manager";
						Index.lgOptionWrong(req, res, info);
					} else {
...
exports.orderDel = function(req, res) {
	...
	if(objBody.status == 0) {
		...
		Order.remove({_id: objBody._id}, function(err, lgOrderRm) {
			if(err) console.log(err);
			res.redirect('/lgOrders')
		})
	} else {
