...
exports.sconts = function(req, res) {
	res.render('./sfer/lger/scont/list', req.body.list)
}
...
exports.scontAdd = function(req, res) {
	Nation.find(function(err, nations) {
		if(err) console.log(err);
		res.render('./sfer/lger/scont/add', {
			title: 'Discount Add',
...
exports.addScont = function(req, res) {
	lgCreateBrand(req, res);
}
lgCreateBrand = function(req, res) {
...
lgCreateScont = function(req, res) {
		...
		.exec(function(err, sconts) {
			...
			if(sconts.length > 0) {
				...
				_object.save(function(err, objSave) {
					...
					res.redirect('/lgScont/' + objSave._id)
				})
			}
		})
	}
...
exports.scont = function(req, res){
	...
	res.render('./sfer/lger/scont/detail', {
...
let rtLgPath = require('path').join(__dirname, "../../../../");
...
exports.scontPdf = function(req, res) {
	...
	// let pug = require('pug');
	if(brand && vendor) {
		let hc = require('pug').renderFile(rtLgPath + 'views/zzPdf/scont/detail.pug', {
			publicPath: "file://"+rtLgPath + 'public',
