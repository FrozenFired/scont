...
exports.cnBrandsFilter = function(req, res, next) {
	let title = '品牌列表';
	let url = "/cnBrands";
...
exports.cnBrands = function(req, res) {
	Cner.find({role: [5, 10, 20]})
	.sort({'role': 1})
	.exec(function(err, users) {
		if(err) console.log(err);
		let list = req.body.list
		list.users = users;
		res.render('./sfer/cner/brand/list', req.body.list)
	})
}
exports.cnBrandsPrint = function(req, res) {
...
// header search
exports.headercnBrand = function(req, res) {
	let objects = req.body.list.objects
	let keyword = req.body.list.keyword
	if(objects && objects.length > 0) {
		if(objects.length == 1) {
			res.redirect("/cnBrand/"+objects[0]._id)
		} else {
			res.redirect('/cnBrands?keyword='+keyword)
		}
	} else {
		info = "没有找到此品牌!"
		Index.cnOptionWrong(req, res, info)
...
exports.cnBrandFilter = function(req, res, next){
...
exports.cnBrand = function(req, res){
