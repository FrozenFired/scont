...
exports.ordersFilter = function(req, res, next) {
	...
	if(req.query && req.query.keytype == "sfer"){
		...
		rpOrderFindSfers(req, res, next, condition);
	} else if(req.query && req.query.keytype == "vder"){
		condition.keytype = "order";
		condition.keyword = "";
		condition.symPul = "$eq";
		condition.condPul = req.query.keyword;
		rpOrderFindVders(req, res, next, condition);
	} else {
		condition.symPul = '$ne';
		condition.condPul = randID;
		rpOrderFindOrders(req, res, next, condition);
	}
}
rpOrderFindSfers = function(req, res, next, condition) {
...
rpOrderFindVders = function(req, res, next, condition) {
...
rpOrderFindOrders = function(req, res, next, condition) {
	Order.count({
...
exports.orders = function(req, res) {
	...
	res.render('./sfer/rper/order/list', list)
}
...
exports.orderAdd =function(req, res) {
	res.render('./sfer/rper/order/add', {
		title: 'Add Order',
	...
	if(isNaN(objBody.price)){
		...
		_object.save(function(err, objSave) {
			if(err) {
				...
				res.redirect('/rpOrder/'+objSave._id)
			}
		})
	}
}

rpAddPayFunc = function(objBody, object) {
...
exports.order = function(req, res) {
	...
	res.render('./sfer/rper/order/detail', {
		title: 'rpOrder Infomation',
		crSfer : req.session.crSfer,
		object: objBody,
		today: today,
		weekday: weekday
...
exports.orderUp = function(req, res) {
	...
	if(objBody.status == 0) {
		...
		res.render('./sfer/rper/order/update', {
			title: 'rpOrder Update',
			action: '/rpOrderUpd',
			crSfer : req.session.crSfer,
			object: objBody,
			today: today,
	...
	Order.findOne({_id: objBody._id}, function(err, object) {
		...
		if(!object) {
			...
			_object.save(function(err, objSave) {
				if(err) {
					console.log(err);
					info = "rpOrderUpd order save error, Please Contact Manager";
					Index.rpOptionWrong(req, res, info);
				} else {
					res.redirect('/rpOrder/'+objSave._id)
				}
	...
	Order.findOne({_id: objBody._id}, function(err, object) {
		...
		if(!object) {
			...
			if(isNaN(parseFloat(objBody.price)) || isNaN(parseFloat(objBody.acPrice)) || isNaN(parseFloat(objBody.saPrice))){
				...
				let _object = _.extend(object, objBody)
				rpUpdPayFunc(req, res, objBody, _object)
...
rpUpdPayFunc = function(req, res, objBody, object) {
	Pay.findOne({_id: object.payAc}, function(err, payAc) {
		if(err) {
			console.log(err);
			info = "rpOrderUpd payAc find id error, Please Contact Manager";
			Index.rpOptionWrong(req, res, info);
		} else if(!payAc) {
			info = "rpOrderUpd payAc find error, Please Contact Manager";
			Index.rpOptionWrong(req, res, info);
		} else {
			...
			payAc.save(function(err, payAcSave) {
				...
				Pay.findOne({_id: object.paySa}, function(err, paySa) {
					if(err) {
						console.log(err);
						info = "rpOrderUpd paySa find id error, Please Contact Manager";
						Index.rpOptionWrong(req, res, info);
					} else if(!paySa) {
						info = "rpOrderUpd paySa find error, Please Contact Manager";
						Index.rpOptionWrong(req, res, info);
					} else {
...
exports.orderDel = function(req, res) {
	...
	if(objBody.status == 0) {
		...
		Order.remove({_id: objBody._id}, function(err, rpOrderRm) {
			if(err) console.log(err);
			res.redirect('/rpOrders')
		})
	} else {
