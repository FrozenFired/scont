...
exports.sconts = function(req, res) {
	res.render('./sfer/rper/scont/list', req.body.list)
}
...
exports.scontAdd = function(req, res) {
	Nation.find(function(err, nations) {
		if(err) console.log(err);
		res.render('./sfer/rper/scont/add', {
			title: 'Discount Add',
...
exports.addScont = function(req, res) {
	rpCreateBrand(req, res);
}
rpCreateBrand = function(req, res) {
...
rpCreateScont = function(req, res) {
		...
		.exec(function(err, sconts) {
			...
			if(sconts.length > 0) {
				...
				_object.save(function(err, objSave) {
					...
					res.redirect('/rpScont/' + objSave._id)
				})
			}
		})
	}
...
exports.scont = function(req, res){
	...
	res.render('./sfer/rper/scont/detail', {
...
let rtRpPath = require('path').join(__dirname, "../../../../");
...
exports.scontPdf = function(req, res) {
	...
	// let pug = require('pug');
	if(brand && vendor) {
		let hc = require('pug').renderFile(rtRpPath + 'views/zzPdf/scont/detail.pug', {
			publicPath: "file://"+rtRpPath + 'public',
