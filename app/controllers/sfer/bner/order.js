...
exports.ordersFilter = function(req, res, next) {
	...
	if(req.query && req.query.keytype == "sfer"){
		...
		bnOrderFindSfers(req, res, next, condition);
	} else if(req.query && req.query.keytype == "vder"){
		condition.keytype = "order";
		condition.keyword = "";
		condition.symPul = "$eq";
		condition.condPul = req.query.keyword;
		bnOrderFindVders(req, res, next, condition);
	} else {
		condition.symPul = '$ne';
		condition.condPul = randID;
		bnOrderFindOrders(req, res, next, condition);
	}
}
bnOrderFindSfers = function(req, res, next, condition) {
...
bnOrderFindVders = function(req, res, next, condition) {
...
bnOrderFindOrders = function(req, res, next, condition) {
	Order.count({
...
exports.orders = function(req, res) {
	...
	res.render('./sfer/bner/order/list', list)
}
...
exports.orderAdd =function(req, res) {
	res.render('./sfer/bner/order/add', {
		title: 'Add Order',
	...
	if(isNaN(objBody.price)){
		...
		_object.save(function(err, objSave) {
			if(err) {
				...
				res.redirect('/bnOrder/'+objSave._id)
			}
		})
	}
}

bnAddPayFunc = function(objBody, object) {
...
exports.order = function(req, res) {
	...
	res.render('./sfer/bner/order/detail', {
		title: 'bnOrder Infomation',
		crSfer : req.session.crSfer,
		object: objBody,
		today: today,
		weekday: weekday
...
exports.orderUp = function(req, res) {
	...
	if(objBody.status == 0) {
		...
		res.render('./sfer/bner/order/update', {
			title: 'bnOrder Update',
			action: '/bnOrderUpd',
			crSfer : req.session.crSfer,
			object: objBody,
			today: today,
	...
	Order.findOne({_id: objBody._id}, function(err, object) {
		...
		if(!object) {
			...
			_object.save(function(err, objSave) {
				if(err) {
					console.log(err);
					info = "bnOrderUpd order save error, Please Contact Manager";
					Index.bnOptionWrong(req, res, info);
				} else {
					res.redirect('/bnOrder/'+objSave._id)
				}
	...
	Order.findOne({_id: objBody._id}, function(err, object) {
		...
		if(!object) {
			...
			if(isNaN(parseFloat(objBody.price)) || isNaN(parseFloat(objBody.acPrice)) || isNaN(parseFloat(objBody.saPrice))){
				...
				let _object = _.extend(object, objBody)
				bnUpdPayFunc(req, res, objBody, _object)
...
bnUpdPayFunc = function(req, res, objBody, object) {
	Pay.findOne({_id: object.payAc}, function(err, payAc) {
		if(err) {
			console.log(err);
			info = "bnOrderUpd payAc find id error, Please Contact Manager";
			Index.bnOptionWrong(req, res, info);
		} else if(!payAc) {
			info = "bnOrderUpd payAc find error, Please Contact Manager";
			Index.bnOptionWrong(req, res, info);
		} else {
			...
			payAc.save(function(err, payAcSave) {
				...
				Pay.findOne({_id: object.paySa}, function(err, paySa) {
					if(err) {
						console.log(err);
						info = "bnOrderUpd paySa find id error, Please Contact Manager";
						Index.bnOptionWrong(req, res, info);
					} else if(!paySa) {
						info = "bnOrderUpd paySa find error, Please Contact Manager";
						Index.bnOptionWrong(req, res, info);
					} else {
...
exports.orderDel = function(req, res) {
	...
	if(objBody.status == 0) {
		...
		Order.remove({_id: objBody._id}, function(err, bnOrderRm) {
			if(err) console.log(err);
			res.redirect('/bnOrders')
		})
	} else {
