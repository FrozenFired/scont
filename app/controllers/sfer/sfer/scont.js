...
exports.sconts = function(req, res) {
	res.render('./sfer/sfer/scont/list', req.body.list)
}
...
exports.scontAdd = function(req, res) {
	Nation.find(function(err, nations) {
		if(err) console.log(err);
		res.render('./sfer/sfer/scont/add', {
			title: 'Discount Add',
...
exports.addScont = function(req, res) {
	sfCreateBrand(req, res);
}
sfCreateBrand = function(req, res) {
...
sfCreateScont = function(req, res) {
		...
		.exec(function(err, sconts) {
			...
			if(sconts.length > 0) {
				...
				_object.save(function(err, objSave) {
					...
					res.redirect('/sfScont/' + objSave._id)
				})
			}
		})
	}
...
exports.scont = function(req, res){
	...
	res.render('./sfer/sfer/scont/detail', {
...
let rtSfPath = require('path').join(__dirname, "../../../../");
...
exports.scontPdf = function(req, res) {
	...
	// let pug = require('pug');
	if(brand && vendor) {
		let hc = require('pug').renderFile(rtSfPath + 'views/zzPdf/scont/detail.pug', {
			publicPath: "file://"+rtSfPath + 'public',
