...
exports.oderFilter = function(req, res, next) {
	let crSfer = req.session.crSfer;
	let id = crSfer._id;
	ObjDB.findOne({_id: id})
	.exec(function(err, object) {
		if(err) { console.log(err); console.log(oderFilter); }
		if(!object) {
			info = "此帐号已经被删除";
...
exports.oderInfo = function(req, res) {
	let object = req.body.object;
	let objBody = new Object();
	objBody.crSfer = req.session.crSfer;
	objBody.object = object;
	objBody.thisAct = "/oderInfo";
	objBody.title = "个人信息";
	res.render('./sfer/oder/self/detail', objBody)
}




exports.oderUp = function(req, res) {
	...
	if(obj._id != req.session.crSfer._id) {
		...
		ObjDB.findOne({_id: obj._id}, function(err, object) {
			if(err) { console.log(err); console.log("oderUp"); }
			if(!object) {
				info = "此用户已经被删除";
				Index.odOptionWrong(req, res, info);
			} else {
