...
exports.cbBrandsFilter = function(req, res, next) {
	let title = '品牌列表';
	let url = "/cbBrands";
...
exports.cbBrands = function(req, res) {
	Cber.find({role: [5, 10, 20]})
	.sort({'role': 1})
	.exec(function(err, users) {
		if(err) console.log(err);
		let list = req.body.list
		list.users = users;
		res.render('./sfer/cber/brand/list', req.body.list)
	})
}
exports.cbBrandsPrint = function(req, res) {
...
// header search
exports.headercbBrand = function(req, res) {
	let objects = req.body.list.objects
	let keyword = req.body.list.keyword
	if(objects && objects.length > 0) {
		if(objects.length == 1) {
			res.redirect("/cbBrand/"+objects[0]._id)
		} else {
			res.redirect('/cbBrands?keyword='+keyword)
		}
	} else {
		info = "没有找到此品牌!"
		Index.cbOptionWrong(req, res, info)
...
exports.cbBrandFilter = function(req, res, next){
...
exports.cbBrand = function(req, res){
	...
	res.render('./sfer/cber/brand/detail', {
		title: '品牌信息',
